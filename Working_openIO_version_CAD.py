...
The class is streamlined to only focus on:
...
THIS VERSION COVNERTS EXIOBASE ENTIRELY TO CAD, TO AVOID TABLES IN MIXED CURRENCY UNITS

This file is used to generate the EEIO matrices from the StatCan SUTs.

It still only works for 2018 until I can get the new stacan GHG data in the right format (email pending).
There are some instances where I need to check if changes made for 2018 still apply to 2019:
- the concordance table in the characterization matrix (for now 2019 = 2018)
- the balance_flows function which selects between different types of particulate matter

The subsequent manipulation of these matrices for research purposes takes place in the following files:
...
class IOTables:
    ...
    def format_tables(self, su_tables, region):
        ...
        #Maintained in case I want to run analyses on previous years
    ...
    def link_merchandise_database_to_openio(self):
        ...
        LEO: Something is happening around here. Imports are getting scaled and converted and I'm not sure what's happening. 
        It's leading to strange new totals for imports
    ...
    def link_international_trade_data_to_exiobase(self):
        ...
        TODO: MY goal is to turn this into CAD entirely, rather than remain in mixed units. What do I need to do to get there?
        S matrix must be converted to CAD
        Z matrix converted to CAD (though not implicated here)
        A matrix? given that it's just coefficients ... no
        Doesnt seem like io.x or io.Y are used.
        
        These following sections create a link between OpenIO and exiobase, by taking the Canadian import data 
        and putting into the exiobase categories so they can be linked to the exiobase A matrix.
        ...
        self.S_exio.iloc[9:] /= 1.5 # because it's impact PER unit currency. Impact per euro to imapct per CAD requires devision.
